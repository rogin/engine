...
public class ConnectServiceUtil {
    ...
    /**
     * Check for new releases. Return notifications for each release that is greater than the current version.
     * 
     * @param serverId
     * @param mirthVersion
     ...
     * @throws Exception should anything fail during the lookup
     */
    public static List<Notification> getNotifications(String serverId, String mirthVersion, Map<String, String> extensionVersions, String[] protocols, String[] cipherSuites) throws ClientException {
        ...
        try {
            Class<?> klass = Class.forName(BrandingConstants.NOTIFICATION_PROVIDER_CLASS);
            provider = (NotificationProvider)klass.getDeclaredConstructor().newInstance();
        } catch (Exception e) {
            throw new ClientException("Bad notification provider: " + BrandingConstants.NOTIFICATION_PROVIDER_CLASS, e);
        }

        return provider.getNotifications(serverId, parsedMirthVersion.get(), extensionVersions, protocols, cipherSuites);
    }
